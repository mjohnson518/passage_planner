...
export class PortAgent {
    ...
    return [
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            draft: {
              type: "number",
              description: "Vessel draft in feet for suitability filtering",
            },
          },
            ...
            text: JSON.stringify(
              {
                ...
                procedures:
                  port.customs.procedures || "Standard CBP procedures apply",
  ...
  private formatPortSummary(port: any): any {
    return {
      ...
      clearance:
        port.clearanceMargin !== undefined
          ? `${port.clearanceMargin.toFixed(1)}ft`
          : undefined,
    };
  }
  ...
  private generatePortRecommendations(port: Port): string[] {
    ...
    if (port.navigation.hazards && port.navigation.hazards.length > 0) {
      recommendations.push(
        `⚠️ Navigation hazards: ${port.navigation.hazards.join(", ")}`,
      );
    }
    ...
    if (port.navigation.tidalRange > 8) {
      recommendations.push(
        `🌊 Large tidal range (${port.navigation.tidalRange}ft) - monitor depth at low water`,
      );
    }
    ...
    if (port.facilities.fuel) {
      recommendations.push(
        `⛽ Fuel available: ${port.facilities.fuel.hours || "Check hours"}`,
      );
    }
  ...
  private getCountryCustomsInfo(country: string): any {
    const customsDB: { [key: string]: any } = {
      ...
      UK: {
        ...
        requirements: {
          ...
          documentation: [
            ...
            "Part I or SSR registration for UK vessels",
          ],
        },
        contacts: {
      ...
      FRANCE: {
        ...
        requirements: {
          ...
          documentation: [
            "Passports/ID cards for all crew",
            "Vessel registration",
      ...
      SPAIN: {
        ...
        requirements: {
          ...
          documentation: [
            ...
            "Title competency (PER/PNB for Spanish waters)",
          ],
        },
        contacts: {
    ...
    return (
      customsDB[country.toUpperCase()] || {
        country,
        message:
          "Customs information not available for this country. Contact local authorities before arrival.",
      }
    );
  }
}
