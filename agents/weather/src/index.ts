...
import {
  MCPValidationError as ValidationError,
  NOAAAPIError,
  toMCPError,
} from "@passage-planner/shared";
...
export function assertWeatherFresh(
  ...
  if (Number.isNaN(ageMs) || ageMs > MAX_WEATHER_AGE_MS) {
    const ageMinutes = Number.isNaN(ageMs)
      ? Infinity
...
export class WeatherAgent extends BaseAgent {
    ...
    super(
      {
        ...
        version: "2.0.0",
        description:
          "Provides real-time marine weather forecasts and safety analysis using NOAA data",
        ...
        tools: [
          {
            ...
            description:
              "Get comprehensive marine weather forecast for a location",
            inputSchema: {
              type: "object",
              properties: {
          ...
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                maxWindSpeed: {
                  type: "number",
                  description: "Maximum safe wind speed in knots",
                },
                maxWaveHeight: {
                  type: "number",
                  description: "Maximum safe wave height in meters",
                },
                minVisibility: {
                  type: "number",
                  description: "Minimum required visibility in nm",
                },
              },
              required: ["latitude", "longitude"],
            },
          ...
          },
          {
            ...
            description:
              "Get real-time wave measurements from NDBC buoys along a route",
            inputSchema: {
              type: "object",
              properties: {
                waypoints: {
          ...
          },
          {
            ...
            description:
              "Get gridded wind and wave forecast data along a route for weather routing",
            inputSchema: {
              type: "object",
              properties: {
                waypoints: {
                ...
                forecastHours: {
                  ...
                  items: { type: "number" },
                  description:
                    "Forecast hours to retrieve (default: [0, 6, 12, 18, 24, 48, 72])",
                },
              },
              required: ["waypoints"],
            },
          },
          {
            name: "health",
            description:
              "Check the health status of the Weather Agent and its dependencies",
            inputSchema: {
              type: "object",
              properties: {},
            },
  ...
  private formatWeatherSummary(forecast: any): string {
    ...
    forecast.periods.slice(0, 6).forEach((period: any) => {
      lines.push(
        `${period.startTime.toLocaleDateString()} ${period.isDaytime ? "☀️" : "🌙"}: ${period.shortForecast}`,
      );
  ...
  private findSafeWindows(
    ...
    while (current < endDate) {
      ...
      const startSafe = this.isWindowSafe(
        startForecast,
        current,
        windowEnd,
        preferences,
      );
      const endSafe = this.isWindowSafe(
        endForecast,
        current,
        windowEnd,
        preferences,
  ...
  private isWindowSafe(
    forecast: any,
    start: Date,
    end: Date,
    preferences: any,
  ): boolean {
    const activeWarnings = forecast.warnings.filter(
      (w: any) =>
        w.onset <= end &&
        w.expires >= start &&
  ...
  private async checkHealth(): Promise<any> {
    ...
    try {
      ...
      const failures =
        health.circuitStates["noaa-forecast"]?.metrics?.failures || 0;
      const successes =
        health.circuitStates["noaa-forecast"]?.metrics?.successes || 0;
