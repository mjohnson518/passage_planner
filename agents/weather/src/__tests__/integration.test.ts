...
describe("WeatherAgent - API Integration - SAFETY CRITICAL", () => {
  ...
  describe("PART C: Tool Functionality", () => {
    describe("Weather Warnings", () => {
      it("should fetch marine weather warnings", async () => {
        ...
        const result = await weatherAgent.handleToolCall(
    ...
    describe("GRIB Data", () => {
      it("should generate valid GRIB download URL", async () => {
        ...
        // NOMADS URL uses "0p50" resolution token (0p25, 0p50, 1p00).
        expect(result.url).toContain("filter_gfs_0p50_deg.pl");
      });
    });
  ...
  describe("Sea State Analysis", () => {
    ...
    it("should provide safety assessment for conditions", async () => {
      ...
      expect(["warning", "dangerous"]).toContain(
        result.periods[0].safetyAssessment.level,
      );
      expect(result.periods[0].safetyAssessment.message).toMatch(
        /strong|rough|gale|warning|dangerous/i,
      );
    });
  });
