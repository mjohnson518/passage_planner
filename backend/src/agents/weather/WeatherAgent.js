...
class WeatherAgent extends BaseAgent_1.BaseAgent {
    noaaApiKey;
    openWeatherApiKey;
    ukmoApiKey;
    constructor(redisUrl, noaaApiKey, openWeatherApiKey) {
        super({
            name: 'weather-agent',
    ...
    async handleToolCall(name, args) {
        switch (name) {
            ...
            case 'find_weather_window':
                return await this.findWeatherWindow(args.latitude, args.longitude, args.duration_hours, args.max_wind_knots, args.max_wave_feet, args.days_ahead);
    ...
    async getMarineForecast(lat, lon, hours = 72) {
        ...
        try {
            return await this.withRetry(async () => {
                ...
                const pointResponse = await axios_1.default.get(`https://api.weather.gov/points/${lat},${lon}`);
                ...
                const marineResponse = await axios_1.default.get(`https://api.openweathermap.org/data/2.5/onecall`, {
                    params: {
                        lat,
                        lon,
    ...
    async getWeatherWarnings(lat, lon, radiusNm = 50) {
        ...
        try {
            return await this.withRetry(async () => {
                ...
                const response = await axios_1.default.get(`https://api.weather.gov/alerts/active`, {
                    params: {
                        point: `${lat},${lon}`,
                        limit: 50
                    }
    ...
    async getGribData(bounds, resolution = '0.5', parameters) {
    ...
    async estimateWaveHeight(windSpeed, lat, lon) {
    ...
    async estimateWavePeriod(windSpeed) {
     ...
     */
    async checkTropicalCyclones(bounds, route) {
        ...
        try {
            return await this.withRetry(async () => {
                ...
                const response = await axios_1.default.get('https://www.nhc.noaa.gov/CurrentStorms.json');
                const cyclones = [];
                        ...
                        if (inBounds || !bounds) {
                            ...
                            if (route) {
                                for (const waypoint of route) {
                                    const distance = this.calculateDistance(stormLat, stormLon, waypoint.latitude, waypoint.longitude);
     ...
     */
    async findWeatherWindow(lat, lon, durationHours, maxWind = 25, maxWave = 6, daysAhead = 7) {
     ...
     */
    async analyzeSeaState(lat, lon, hours = 48) {
        ...
        return {
            ...
            summary: {
                ...
                safestPeriod: seaStates.reduce((best, current) => current.waveHeight.feet < best.waveHeight.feet ? current : best),
                roughestPeriod: seaStates.reduce((worst, current) => current.waveHeight.feet > worst.waveHeight.feet ? current : worst),
            },
        };
    }
    /**
     ...
     */
    waveHeightToDouglasScale(heightFeet) {
     ...
     */
    assessSeaConditions(windKnots, waveHeightFeet) {
        if (windKnots < 10 && waveHeightFeet < 2)
            return 'Excellent - calm conditions';
        if (windKnots < 15 && waveHeightFeet < 4)
            return 'Good - comfortable sailing';
        if (windKnots < 20 && waveHeightFeet < 6)
            return 'Fair - moderate conditions';
        if (windKnots < 25 && waveHeightFeet < 8)
            return 'Challenging - experienced crew recommended';
        if (windKnots < 35 && waveHeightFeet < 12)
            return 'Rough - only experienced crews';
        return 'Severe - consider sheltering';
    }
    /**
     * Assess safety for small craft
     */
    assessSeaSafety(windKnots, waveHeightFeet) {
        if (windKnots >= 35 || waveHeightFeet >= 12) {
            return {
                level: 'dangerous',
     ...
     */
    parseStormCategory(classification) {
        const lower = classification.toLowerCase();
        if (lower.includes('category 5') || lower.includes('cat 5'))
            return 5;
        if (lower.includes('category 4') || lower.includes('cat 4'))
            return 4;
        if (lower.includes('category 3') || lower.includes('cat 3'))
            return 3;
        if (lower.includes('category 2') || lower.includes('cat 2'))
            return 2;
        if (lower.includes('category 1') || lower.includes('cat 1'))
            return 1;
        if (lower.includes('hurricane'))
            return 1;
        if (lower.includes('tropical storm'))
            return 0;
        return -1;
    }
    /**
