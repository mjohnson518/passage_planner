...
export interface TestRoute {
  start: TestCoordinate;
  end: TestCoordinate;
  expectedDistance?: number; // nautical miles
  expectedBearing?: number; // degrees
  challenges?: string[];
}
 ...
 */
export const TEST_ROUTES: Record<string, TestRoute> = {
  BOSTON_TO_PORTLAND: {
    start: TEST_COORDINATES.BOSTON,
    end: TEST_COORDINATES.PORTLAND_ME,
  ...
  BOSTON_TO_NEWPORT: {
    start: TEST_COORDINATES.BOSTON,
    end: TEST_COORDINATES.NEWPORT_RI,
  ...
  DATELINE_CROSSING: {
    start: TEST_COORDINATES.DATELINE_WEST,
    end: TEST_COORDINATES.DATELINE_EAST,
    expectedDistance: 60, // nm at 20° latitude (approximate)
    challenges: ['dateline', 'coordinate_normalization']
  },
  
  EQUATOR_CROSSING: {
    start: TEST_COORDINATES.EQUATOR_WEST,
    end: TEST_COORDINATES.EQUATOR_EAST,
  ...
  ANTIPODAL_ROUTE: {
    start: TEST_COORDINATES.ANTIPODAL_A,
    end: TEST_COORDINATES.ANTIPODAL_B,
 ...
 */
export function getTestCoordinatePair(name: keyof typeof TEST_COORDINATES) {
  const coord = TEST_COORDINATES[name];
  return {
    lat: coord.lat,
    lon: coord.lon
  };
 ...
 */
export function getTestRoute(name: keyof typeof TEST_ROUTES) {
