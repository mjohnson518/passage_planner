...
class AnalyticsService {
    ...
    async trackEvent(event) {
        try {
            // Store in database
            await this.db.query(`INSERT INTO analytics_events 
    ...
    async getUserJourney(userId) {
        const result = await this.db.query(`SELECT event_name, properties, created_at
       FROM analytics_events
       WHERE user_id = $1
    ...
    async getFeatureUsage(days = 30) {
        const result = await this.db.query(`SELECT event_name, COUNT(*) as count
       FROM analytics_events
     ...
     */
    async getCohortRetention(cohortMonth) {
        const retention = [];
        for (let week = 0; week <= 12; week++) {
            const result = await this.db.query(`WITH cohort AS (
        ...
        WHERE e.created_at >= $1::date + INTERVAL '${week} weeks'
          AND e.created_at < $1::date + INTERVAL '${week + 1} weeks'`, [cohortMonth]);
