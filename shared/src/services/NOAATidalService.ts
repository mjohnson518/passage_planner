...
export interface TidalData {
  ...
  // Timestamp at which this prediction set was fetched from NOAA. Safety-
  // critical: downstream consumers (agents/tidal) assert freshness against
  // MAX_TIDAL_AGE_MS before returning tides to callers. Populated on fresh
  // fetch; for fallback-cache paths, rehydrated from the cached payload's
  // own fetchedAt so staleness is measured from the original fetch, not the
  // cache-replay moment.
  fetchedAt: Date;
}
...
export class NOAATidalService {
  ...
  private readonly TIDES_API_BASE =
    "https://api.tidesandcurrents.noaa.gov/api/prod/datagetter";
  private readonly STATIONS_API =
  ...
  constructor(cache: CacheManager, logger?: Logger) {
    ...
    axiosRetry(this.httpClient, {
      ...
      onRetry: (retryCount, error) => {
        this.logger.warn(
          `Tidal API retry attempt ${retryCount} for ${error.config?.url}`,
        );
      },
    });
    ...
    this.logger.info(
      "NOAA Tidal Service initialized - Critical component ready",
    );
  }
  ...
  async findNearestStations(
    ...
    const cachedData =
      await this.cache.getWithMetadata<TidalStation[]>(cacheKey);
    if (cachedData && cachedData.value) {
      this.logger.debug(
        { age: cachedData.age, ttl: cachedData.ttl },
        "Returning cached tidal stations",
      );
    ...
    try {
      ...
      const allStations: TidalStation[] = await this.stationsBreaker
        .fire()
        ...
        .catch(async (error) => {
          ...
          this.logger.warn(
            { error: error.message },
            "Circuit breaker open for stations, checking fallback cache",
          );
          const fallbackCache = await this.cache.get<TidalStation[]>(
            "tidal:stations:fallback",
          );
          if (fallbackCache) {
  ...
  async getTidalPredictions(
    ...
    if (cachedData && cachedData.value) {
      ...
      // JSON round-trip through the cache turns Date into string; rehydrate so
      // downstream freshness assertions work on a real Date instance.
      const cachedValue = cachedData.value;
      if (cachedValue.fetchedAt && !(cachedValue.fetchedAt instanceof Date)) {
        cachedValue.fetchedAt = new Date(
          cachedValue.fetchedAt as unknown as string,
        );
      }
      return cachedValue;
    }
    ...
    try {
      ...
      // Track whether predictions came from a circuit-breaker fallback cache,
      // and if so, the original fetch time — so TidalData.fetchedAt reflects
      // when the predictions were actually retrieved from NOAA, not when this
      // fallback path happened to run. Without this, a stale fallback would
      // sail past the downstream MAX_TIDAL_AGE_MS check.
      let fallbackFetchedAt: Date | null = null;
      ...
      const [predictions, extremes, station] = await Promise.all([
        this.predictionsBreaker
          .fire(stationId, beginDate, range)
          .then((result: any) => result)
          .catch(async (error) => {
            // If circuit is open, try to return cached data
            this.logger.warn(
              { error: error.message },
              "Circuit breaker open for predictions, checking fallback cache",
            );
            const fallbackCache = await this.cache.get<{
              predictions: TidalPrediction[];
              fetchedAt: string | Date;
            }>(`tidal:predictions:fallback:${stationId}`);
            if (fallbackCache && Array.isArray(fallbackCache.predictions)) {
              this.logger.info("Using fallback cached predictions data");
              fallbackFetchedAt =
                fallbackCache.fetchedAt instanceof Date
                  ? fallbackCache.fetchedAt
                  : new Date(fallbackCache.fetchedAt);
              return fallbackCache.predictions;
            }
            throw error; // Re-throw if no cache available
          }),
      ...
      const fetchedAt = fallbackFetchedAt ?? new Date();
      ...
      // Store fallback cache for circuit breaker. Persist the fetch timestamp
      // alongside the payload so a future fallback-path read can preserve it
      // and the tidal-agent freshness assertion measures staleness from the
      // original NOAA fetch, not from the fallback-replay moment.
      await this.cache.setWithTTL(
        `tidal:predictions:fallback:${stationId}`,
        { predictions, fetchedAt },
      ...
      this.logger.error(
        { error, stationId },
        "Failed to get tidal predictions",
      );
      throw new Error(
        `Unable to retrieve tidal predictions for station ${stationId}`,
      );
    }
  }
  ...
  async getCurrentPredictions(
    ...
    try {
      ...
      if (data.error) {
        this.logger.warn(
          { stationId },
          "Current predictions not available for station",
        );
        return [];
      }
      ...
      this.logger.warn(
        { error, stationId },
        "Failed to get current predictions",
      );
      return [];
    }
  }
  ...
  async calculateTidalWindows(
    ...
    const tidalData = await this.getTidalPredictions(
      stationId,
      startDate,
      endDate,
    );
    ...
    for (const prediction of tidalData.predictions) {
      const heightOk =
        prediction.height >= requirements.minTideHeight &&
        (!requirements.maxTideHeight ||
          prediction.height <= requirements.maxTideHeight);
      ...
      if (heightOk) {
        if (!currentWindow) {
          ...
          currentWindow.minHeight = Math.min(
            currentWindow.minHeight,
            prediction.height,
          );
          currentWindow.maxHeight = Math.max(
            currentWindow.maxHeight,
            prediction.height,
          );
        }
      } else {
   ...
   */
  private classifyCurrentType(
    velocity: number,
  ): CurrentPrediction["type"] | undefined {
