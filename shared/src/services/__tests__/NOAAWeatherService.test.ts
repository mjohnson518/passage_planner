...
} from "../../testing/helpers/assertions";
...
describe("NOAAWeatherService - SAFETY-CRITICAL Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    mockClient.getWeatherForecast.mockResolvedValue(
      MOCK_FORECAST_BOSTON.properties,
    );
    ...
    mockClient.getGridPoint.mockResolvedValue({
      ...
      state:
        MOCK_GRID_POINT_BOSTON.properties.relativeLocation.properties.state,
      timeZone: MOCK_GRID_POINT_BOSTON.properties.timeZone,
    });

    // Create service with injected mock client
    weatherService = new NOAAWeatherService(
      cache,
      logger,
      mockClient as unknown as NOAAAPIClient,
    );
  });
  ...
  describe("Grid Point Lookup Integration", () => {
    ...
    it("should use cached grid point on subsequent calls", async () => {
      ...
      expect(
        mockClient.getWeatherForecast.mock.calls.length,
  ...
  describe("Forecast Retrieval & Parsing", () => {
    ...
    it("should cache forecast for 1 hour (matches CLAUDE.md stale-data contract)", async () => {
      // Default mocks already configured in beforeEach
      await weatherService.getMarineForecast(42.3601, -71.0589, 7);
      ...
      // TTL is derived from WEATHER_CACHE_TTL_S = WEATHER_REJECT_AGE_MS / 1000 = 3600s.
      // The contract is non-negotiable: weather older than 1h is stale and must not
      // be served; the cache TTL is intentionally tied to the rejection threshold so
      // the two cannot drift apart.
      const metadata = await cache.getWithMetadata(
        "weather:forecast:42.3601,-71.0589:7",
      );
      if (metadata) {
        expect(metadata.ttl).toBeGreaterThan(3500);
        expect(metadata.ttl).toBeLessThanOrEqual(3600);
      }
    });
    ...
    it("should detect and handle gale force winds", async () => {
      ...
      mockClient.getWeatherForecast.mockResolvedValueOnce(
        MOCK_FORECAST_GALE_WARNING.properties,
      );
  ...
  describe("Data Validation & Freshness", () => {
    ...
    it("should reject stale forecast data", async () => {
      mockClient.getWeatherForecast.mockResolvedValueOnce(
        MOCK_FORECAST_STALE.properties,
      );
  ...
  describe("Retry Logic Integration", () => {
    it("should handle 503 Service Unavailable through circuit breaker", async () => {
      ...
      const forecast = await weatherService.getMarineForecast(
        ...
        -71.059,
        7,
      ); // Different coords to avoid cache
  ...
  describe("Weather Warning Integration", () => {
    it("should include gale warnings in forecast", async () => {
      ...
      expect(forecast.warnings.length).toBeGreaterThan(0);
      const galeWarning = forecast.warnings.find(
        (w) => w.headline?.includes("Gale") || w.type === "gale",
      );
  ...
  describe("Multiple Geographic Regions", () => {
    ...
    it("should handle different coordinates without interference", async () => {
      ...
      const boston = await weatherService.getMarineForecast(
        42.3601,
        -71.0589,
        7,
      );
      ...
      const portland = await weatherService.getMarineForecast(
        43.6591,
        -70.2568,
        7,
      );
