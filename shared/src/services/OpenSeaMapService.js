...
exports.OpenSeaMapService = void 0;
const axios_1 = __importDefault(require("axios"));
class OpenSeaMapService {
    logger;
    // Chart tile servers
    OSM_BASE_URL = 'https://tile.openstreetmap.org';
    SEAMARK_URL = 'https://tiles.openseamap.org/seamark';
    DEPTH_URL = 'http://tiles.openseamap.org/depth';
     ...
     */
    async getNavigationHazards(bounds) {
        try {
            const query = `
        [out:json][timeout:25];
        (
     ...
     */
    async getNavigationAids(bounds) {
        try {
            const query = `
        [out:json][timeout:25];
        (
      ...
      `;
            ...
            response.data.elements.forEach((element) => {
                ...
                if (!['lighthouse', 'beacon', 'buoy'].includes(type))
                    return;
                const aid = {
                    id: `${element.type}/${element.id}`,
                    type: type,
     ...
     */
    async validateRoute(waypoints, draft // vessel draft in meters
    ) {
        const warnings = [];
        const foundHazards = [];
        ...
        for (let i = 0; i < waypoints.length - 1; i++) {
            ...
            hazards.forEach(hazard => {
                const distance = this.distanceToLine(hazard.position, start, end);
                // If hazard is within 0.1nm (185m) of route
                if (distance < 0.1) {
                    foundHazards.push(hazard);
                    if (hazard.type === 'rock' || hazard.type === 'wreck') {
                        warnings.push(`${hazard.type} near route segment ${i + 1}: ${hazard.name || 'unnamed'}`);
                    }
                    if (hazard.type === 'shoal' && hazard.depth && hazard.depth < draft + 2) {
                        warnings.push(`Insufficient depth at ${hazard.name || 'shoal'}: ${hazard.depth}m (vessel draft: ${draft}m)`);
                    }
                }
            });
        }
     ...
     */
    async getDepthProfile(waypoints) {
     ...
     */
    getChartTileUrl(latitude, longitude, zoom, layer = 'seamark') {
     ...
     */
    calculateRouteBounds(waypoints) {
...
exports.OpenSeaMapService = OpenSeaMapService;
//# sourceMappingURL=OpenSeaMapService.js.map