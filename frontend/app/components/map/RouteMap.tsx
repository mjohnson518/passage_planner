...
const EMPTY_WAYPOINTS: NonNullable<RouteMapProps["waypoints"]> = [];
...
export default function RouteMap({
  ...
  const [activeLayer, setActiveLayer] = useState<
    "street" | "nautical" | "satellite"
  >("nautical");

  // The map is initialized exactly once on mount; capture the initial view in
  // refs so the init effect can run with an empty dependency list without the
  // linter flagging `center`/`zoom` as missing (re-running init would destroy
  // and rebuild the map, which is not the intended behavior).
  ...
  useEffect(() => {
    ...
    const satelliteLayer = L.tileLayer(
      ...
      {
        attribution:
          "&copy; Esri &mdash; Sources: Esri, DigitalGlobe, Earthstar Geographics",
        maxZoom: 18,
      },
    );
    ...
    const noaaChartLayer = L.tileLayer(
