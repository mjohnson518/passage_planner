        ...
        <div className="space-y-6 py-4">
          ...
          <div className="rounded-lg bg-muted/30 p-3">
            <p className="text-sm text-muted-foreground">
              ...
              {passage.name.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_passage.
              {format}
            </p>
            <p className="text-sm text-muted-foreground mt-1">
              <strong>Contains:</strong> {(passage.waypoints?.length ?? 0) + 2}{" "}
              waypoints, {(passage.distance ?? 0).toFixed(1)} nm
            </p>
          </div>
        </div>
