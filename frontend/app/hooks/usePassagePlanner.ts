...
export function usePassagePlanner(): UsePassagePlannerReturn {
  ...
  const planPassage = useCallback(
    async (request: PassagePlanRequest) => {
      if (!user) {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/mcp/tools/call`,
          {
            method: "POST",
            credentials: "include",
            headers: {
  ...
  const pollForUpdates = async (requestId: string) => {
    const pollInterval = setInterval(async () => {
      try {
        ...
        logger.error("Passage-plan polling error", { error: String(err) });
      }
    }, 2000);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 403) {
            throw new Error(
              `${format.toUpperCase()} export requires a premium subscription`,
            );
          }
