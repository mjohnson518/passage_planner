...
export async function POST(request: NextRequest) {
  try {
        ...
        return NextResponse.json(
          {
            ...
            message:
              "You need an active subscription to access the billing portal. Start a subscription from the pricing page.",
          },
          { status: 404 },
        );
      }
    ...
    serverLogger.error("Customer portal creation error", {
      error: String(error),
    });
    return NextResponse.json(
