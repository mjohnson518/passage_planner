...
// API routes to cache
const API_ROUTES = ["/api/auth/session", "/api/user/profile", "/api/passages"];
        ...
        if (isWeatherOrTidal) {
          ...
          if (cachedDate) {
            ...
            if (cacheAgeMs > ONE_HOUR_MS) {
              ...
              headers.set(
                "X-Cache-Age-Minutes",
                String(Math.round(cacheAgeMs / 60000)),
              );
...
async function syncPassages() {
  try {
    ...
    // Send each passage to the server concurrently — passages are independent
    await Promise.all(
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(self.registration.showNotification("Helmwise", options));
});
