...
- Third batch of logger-shim migration: the user-facing page code — `fleet/page.tsx` (3), `weather/page.tsx` (2), `planner/page.tsx` (1 — passage-planning failure path, now tagged with departure/destination), `passages/page.tsx` (1), `passages/[id]/PassageDetailClient.tsx` (1), `dashboard/page-optimized.tsx` (1) — plus the six dashboard widgets that render on those pages: `FleetAnalyticsDashboard`, `AddVesselDialog`, `SharePassageDialog`, `WeatherWidget`, `RecentPassages`, `AnalyticsDashboard`. 15 client-side `console.error` sites now go through the logger. Remaining follow-up: error boundaries (`ErrorBoundary`, `error.tsx` files), auxiliary client components (`FeedbackWidget`, `InteractiveMap`, `OnboardingFlow`, `StripeCheckout`, `LocationAutocomplete`, `api-docs`), socket/auth contexts, `lib/performance.ts`, and the server-side `api/**/route.ts` + `auth/callback/route.ts` handlers (those run on Node and will use Pino in a later batch rather than the browser shim).
