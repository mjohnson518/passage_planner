...
export class Orchestrator {
  ...
  private async planPassage(request: any): Promise<any> {
    ...
    try {
      ...
      const route = await this.agents["route"].handleToolCall(
        "calculate_route",
        {
      ...
      const weatherPromises = route.waypoints.map((wp: any) =>
        this.agents["weather"]
          .handleToolCall("get_marine_forecast", {
      ...
      let tidalData = null;
      try {
        tidalData = await this.agents["tidal"].handleToolCall(
          "get_tide_predictions",
          {
            ...
            end_date: new Date(
              new Date(request.departure.time).getTime() +
                7 * 24 * 60 * 60 * 1000,
            ).toISOString(),
          },
        );
      ...
      if (request.userId) {
        ...
        if (!saveResult.saved) {
          ...
          passagePlan.summary.warnings.push(`WARNING: ${saveResult.error}`);
        }
      }
      ...
      try {
        ...
        for (const w of weather) {
          ...
          if (Array.isArray(w)) {
            for (const f of w) {
              if (f?.windSpeed != null)
                windSpeeds.push(Number(f.windSpeed) || 0);
              if (f?.waveHeight != null)
                waveHeights.push(Number(f.waveHeight) || 0);
            }
          }
          // If w is an object with windSpeed directly
            ...
            if (w.waveHeight != null)
              waveHeights.push(Number(w.waveHeight) || 0);
          }
        }
        ...
        const maxWaveHeight =
          waveHeights.length > 0 ? Math.max(...waveHeights) : 0;
        ...
        if (windSpeeds.length > 0) {
          ...
          if (avgWindSpeed < 5) {
            recommendations.push(
              "Light winds expected - consider motor sailing",
            );
          } else if (avgWindSpeed > 20) {
            recommendations.push(
              "Strong winds forecast - reef early and monitor conditions",
            );
          }
        }
      } catch {
  ...
  private async savePassage(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const { data, error } = await this.supabase.from("passages").insert({
          id: passagePlan.id,
        ...
        if (isLastAttempt) {
          ...
          return {
            saved: false,
            error:
              "Failed to save passage to database. Please export your passage plan as a backup.",
          };
        }
    ...
    this.app.get("/health", async (req, res) => {
      try {
        ...
        for (const [name, agent] of Object.entries(this.agents)) {
          try {
            const agentHealth = await this.redis.hgetall(
              `agent:health:${name}-agent`,
            );
            health.agents[name] = {
